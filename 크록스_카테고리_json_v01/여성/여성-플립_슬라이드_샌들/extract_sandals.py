...
def extract_sandals_data(html_file):
    """HTML 파일에서 샌들 상품 데이터를 추출하여 JSON 형태로 변환"""
    ...
    for idx, li in enumerate(product_cards):
        ok_card = li.find('div', class_='ok-card')
        if not ok_card:
            continue
            
        product = {}
        product["번호"] = start_number + idx
        ...
        if price_section:
            ...
            # 가격 범위가 있는 경우 처리 (₩34,900 ~ ₩49,900 같은 형태)
        ...
        print(f"상품 {product['번호']} 처리 완료: {product['상품명']}")
...
if __name__ == "__main__":
    # HTML 파일 경로
    html_file = "여성-샌들.html"
    
    # 출력 JSON 파일 경로
    output_file = "여성-샌들.json"
    
    print("HTML 파일에서 샌들 상품 데이터 추출 시작...")
    
    try:
        # 상품 데이터 추출
        products = extract_sandals_data(html_file)
