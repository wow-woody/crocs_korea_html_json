...
def process_sandals_images_and_update_json():
    """샌들 이미지 다운로드 및 JSON 업데이트"""
    
    # JSON 파일 읽기
    json_file = "여성-샌들.json"
...
if __name__ == "__main__":
    print("🖼️  샌들 상품 이미지 다운로드 시작...\n")
    
    try:
        downloaded, total = process_sandals_images_and_update_json()
        
        if downloaded > 0:
            print(f"\n✅ 샌들 이미지 다운로드 및 JSON 업데이트가 완료되었습니다!")
            print(f"   images 폴더에 {downloaded}개의 이미지가 추가로 저장되었습니다.")
        else:
            print("\n⚠️  다운로드된 이미지가 없습니다.")
            
    except FileNotFoundError:
        print("❌ JSON 파일을 찾을 수 없습니다: 여성-샌들.json")
