...
// HTML 파일 읽기 (여성-클로그_2.html)
const htmlContent = fs.readFileSync('여성-클로그_2.html', 'utf-8');
    ...
    if (cardMatch) {
        ...
        try {
            ...
            if (swatchesMatch) {
                ...
                // linear-gradient도 확인
                const gradientMatches = [...swatchesMatch[1].matchAll(/<li class="ok-card__swatch-color"[^>]*style="background:\s*linear-gradient[^"]*"[^>]*>/g)];
                gradientMatches.forEach(() => {
                    colors.push('그라데이션');
                });
            }
            ...
            if (priceMatch) {
                ...
                // 할인가 할인율 (.ok-card__price-value--bold .ok-card__price-value--sale)
                const saleMatches = [...priceContent.matchAll(/<span[^>]*class="[^"]*ok-card__price-value[^"]*ok-card__price-value--bold[^"]*ok-card__price-value--sale[^"]*"[^>]*>([^<]+)<\/span>/g)];
                ...
                const boldMatches = [...priceContent.matchAll(/<span[^>]*class="[^"]*ok-card__price-value[^"]*ok-card__price-value--bold[^"]*"[^>]*>([^<]+)<\/span>/g)];
            ...
            // 상품 데이터 구성 (크록스_상품목록_최종.json 형식에 맞춤)
            const product = {
                '번호': productIndex,
                '상품 ID': productId,
                '상품명': productName,
